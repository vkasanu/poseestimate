...
interpreter = tf.lite.Interpreter(model_path='modelthunder.tflite')
...
while cap.isOpened():
    ret,frame =cap.read()
    #reshaping the dimensions of the image
    img = frame.copy()
    ...
    fps=1/(current-past)
    past = current
    cv2.putText(frame, str(int(fps)), (0, 256), cv2.FONT_HERSHEY_COMPLEX,3,(0,0,255),4)
